...
try:
    memory = MemoryStore(MEMORY_BACKENDS)
    print('[CORTEX] Memory Store loaded — %d backends' % len(memory.backends))
except Exception as e:
    print('[CORTEX] Memory Store FAILED: %s' % str(e))
    memory = None
    ...
    def do_POST(self):
        ...
        # ═══════════════════════════════════════════
            ...
            self._json_response({
                ...
                'date': today,
                'size_bytes': len(compressed),
                'message': f'Archived {total_archived} memories to IPFS. Mind is clear.',
            })
